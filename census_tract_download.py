...
def download_nyc_census_tracts():
    """
    Download census tract boundaries for all NYC boroughs.
    """
...
def get_acs_variables(api_key, year="2021"):
    ...
    try:
        ...
        estimate_vars = [var for var in all_vars 
                        ...
                        var not in {"state", "county", "tract", "for", "in"}]
        ...
        if batch_data:
            if not all_data:
                ...
                for row_idx in range(1, len(batch_data)):
                    all_data[row_idx].extend(batch_data[row_idx][1:-3])  # Exclude NAME and geo columns
...
if __name__ == "__main__":
    ...
    gdf_boundaries = download_nyc_census_tracts()
    ...
    if gdf_boundaries is not None:
        ...
        if variables:
            ...
            if all_acs_data:
                ...
                # Print summary
                print(f"\nSummary:")
                print(f"Number of census tracts: {len(merged_gdf)}")
